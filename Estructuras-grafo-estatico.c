...
#define MAX_Personas_por_territorio 25 //Máximo de personas por territorio
...
int main(int argc, char const *argv[])
{
    ...
    //Prueba de agregar territorios e imprimirlos

    addterritorio(&sistema,"CDMX");
    addterritorio(&sistema,"Guadalajara");
    addterritorio(&sistema,"Puebla");
    addterritorio(&sistema,"Veracruz");
    addterritorio(&sistema,"Monterrey");
    addterritorio(&sistema,"Sinaloa");

    mostrar_territorios(sistema);


    addpersonas(&sistema,1,1,"Eduardo",0.746,1.2);
    addpersonas(&sistema,2,1,"Leonardo",0.746,1.2);
    addpersonas(&sistema,1,2,"Karla",0.746,1.2);
...
void mostrar_persona(sistema sistema){
    if (sistema.numterritorios >= MAX_Territorios || sistema.numterritorios == 0)
    {
